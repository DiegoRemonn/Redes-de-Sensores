...
// Activo o no el acelerómetro con característica propia
...
bool accelerometerSwitch = false;
...
void setup() {
  ...
  accelerometerService.addCharacteristic(ble_accelerometerSwitch);

  ble_accelerometerSwitch.writeValue(accelerometerSwitch);
...
void loop() {
  ...
  if (central) {
    ...
    while (central.connected()) {
      if (ble_accelerometerSwitch.written()) {
        accelerometerSwitch = ble_accelerometerSwitch.value();
      }
      if (send == true) {
...
void readValues() {
  // Compruebo que puedo enviar datos sino envio 0.0
  if (accelerometerSwitch) {
